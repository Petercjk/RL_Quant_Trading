...
class StockTradingEnv(gym.Env):
    ...
    State:
        [cash, prices (N), holdings (N), technical indicators (N * K, flatten)]
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # Debug/analysis fields for backtest diagnostics.
        self.last_action_raw = np.zeros(self.stock_dim, dtype=np.float32)
        self.last_action_shares = np.zeros(self.stock_dim, dtype=np.int32)
        self.last_trade_shares = np.zeros(self.stock_dim, dtype=np.int32)
    ...
    def step(self, action):
        ...
        for i in range(self.stock_dim):
            if actions[i] < 0:
                sold = self._sell(i, abs(actions[i]))
                executed_shares[i] -= sold
                trade_fees[i] += sold * trade_prices[i] * self.sell_cost_pct[i]

        for i in range(self.stock_dim):
            if actions[i] > 0:
                bought = self._buy(i, actions[i])
                executed_shares[i] += bought
                trade_fees[i] += bought * trade_prices[i] * self.buy_cost_pct[i]

        self.last_action_raw = action.copy()
        self.last_action_shares = actions.copy()
    ...
    def _update_market_data(self):
        ...
        self.current_tics = self.data.tic.values.tolist()
    ...
    def _get_state(self):
        return np.concatenate([[self.cash], self.prices, self.holdings, self.techs])
